...
class CommitteeLogHistoryDashboardPage extends ConsumerWidget {
  const CommitteeLogHistoryDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(committeeLogHistoryListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('LogHitorys (Committee)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/committee/log-history/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<CommitteeLogHistory>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: CommitteeLogHistoryDataSource(list, context),
...
class CommitteeLogHistoryDataSource extends DataGridSource {
  CommitteeLogHistoryDataSource(List<CommitteeLogHistory> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<CommitteeLogHistory>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as CommitteeLogHistory;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/committee/log-history/${item.id}'),
        child: Container(
