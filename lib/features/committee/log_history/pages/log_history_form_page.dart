...
class CommitteeLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const CommitteeLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<CommitteeLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<CommitteeLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(committeeLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(committeeLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(committeeLogHistoryListProvider);
