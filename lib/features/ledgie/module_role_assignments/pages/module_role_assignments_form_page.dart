...
class LedgieModuleRoleAssignmentFormPage extends ConsumerStatefulWidget {
  final String? id;
  const LedgieModuleRoleAssignmentFormPage({super.key, this.id});

  @override
  ConsumerState<LedgieModuleRoleAssignmentFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<LedgieModuleRoleAssignmentFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(ledgieModuleRoleAssignmentRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(ledgieModuleRoleAssignmentRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(ledgieModuleRoleAssignmentListProvider);
