...
class LedgieFileFormPage extends ConsumerStatefulWidget {
  final String? id;
  const LedgieFileFormPage({super.key, this.id});

  @override
  ConsumerState<LedgieFileFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<LedgieFileFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(ledgieFileRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(ledgieFileRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(ledgieFileListProvider);
