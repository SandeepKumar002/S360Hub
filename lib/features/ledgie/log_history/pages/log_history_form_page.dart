...
class LedgieLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const LedgieLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<LedgieLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<LedgieLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(ledgieLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(ledgieLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(ledgieLogHistoryListProvider);
