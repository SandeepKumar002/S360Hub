...
class ContracforceContractorStaffDocumentDetailPage extends ConsumerWidget {
  final String id;
  const ContracforceContractorStaffDocumentDetailPage({super.key, required this.id});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final itemAsync = ref.watch(contracforceContractorStaffDocumentDetailProvider(id));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => context.push('/contracforce/contractor-staff-documents/$id/edit'),
          ),
        ],
      ),
      body: AsyncValueWidget<ContracforceContractorStaffDocument>(
        value: itemAsync,
        data: (item) {
          return ListView(
            padding: const EdgeInsets.all(16),
            children: [
              ListTile(title: Text('ContractorStaffId'), subtitle: Text(item.contractorStaffId.toString())),
