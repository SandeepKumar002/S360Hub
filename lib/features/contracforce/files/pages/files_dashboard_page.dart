...
class ContracforceFileDashboardPage extends ConsumerWidget {
  const ContracforceFileDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(contracforceFileListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Files (Contracforce)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/contracforce/files/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<ContracforceFile>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: ContracforceFileDataSource(list, context),
...
class ContracforceFileDataSource extends DataGridSource {
  ContracforceFileDataSource(List<ContracforceFile> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<ContracforceFile>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as ContracforceFile;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/contracforce/files/${item.id}'),
        child: Container(
