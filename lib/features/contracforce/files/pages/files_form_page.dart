...
class ContracforceFileFormPage extends ConsumerStatefulWidget {
  final String? id;
  const ContracforceFileFormPage({super.key, this.id});

  @override
  ConsumerState<ContracforceFileFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<ContracforceFileFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(contracforceFileRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(contracforceFileRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(contracforceFileListProvider);
