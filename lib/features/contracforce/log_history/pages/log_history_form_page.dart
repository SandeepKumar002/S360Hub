...
class ContracforceLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const ContracforceLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<ContracforceLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<ContracforceLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(contracforceLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(contracforceLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(contracforceLogHistoryListProvider);
