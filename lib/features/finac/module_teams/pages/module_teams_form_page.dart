...
class FinacModuleTeamFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacModuleTeamFormPage({super.key, this.id});

  @override
  ConsumerState<FinacModuleTeamFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacModuleTeamFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacModuleTeamRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacModuleTeamRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacModuleTeamListProvider);
