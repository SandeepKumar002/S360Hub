...
class FinacModuleRoleFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacModuleRoleFormPage({super.key, this.id});

  @override
  ConsumerState<FinacModuleRoleFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacModuleRoleFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacModuleRoleRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacModuleRoleRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacModuleRoleListProvider);
