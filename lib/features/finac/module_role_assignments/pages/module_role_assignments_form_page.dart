...
class FinacModuleRoleAssignmentFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacModuleRoleAssignmentFormPage({super.key, this.id});

  @override
  ConsumerState<FinacModuleRoleAssignmentFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacModuleRoleAssignmentFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacModuleRoleAssignmentRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacModuleRoleAssignmentRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacModuleRoleAssignmentListProvider);
