...
class FinacModuleUserFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacModuleUserFormPage({super.key, this.id});

  @override
  ConsumerState<FinacModuleUserFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacModuleUserFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacModuleUserRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacModuleUserRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacModuleUserListProvider);
