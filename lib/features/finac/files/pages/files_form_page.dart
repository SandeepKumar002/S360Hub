...
class FinacFileFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacFileFormPage({super.key, this.id});

  @override
  ConsumerState<FinacFileFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacFileFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacFileRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacFileRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacFileListProvider);
