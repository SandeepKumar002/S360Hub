...
class FinacFileDashboardPage extends ConsumerWidget {
  const FinacFileDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(finacFileListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Files (Finac)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/finac/files/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<FinacFile>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: FinacFileDataSource(list, context),
...
class FinacFileDataSource extends DataGridSource {
  FinacFileDataSource(List<FinacFile> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<FinacFile>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as FinacFile;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/finac/files/${item.id}'),
        child: Container(
