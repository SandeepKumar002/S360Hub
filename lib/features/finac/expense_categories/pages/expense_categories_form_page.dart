...
class FinacExpenseCategorieFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacExpenseCategorieFormPage({super.key, this.id});

  @override
  ConsumerState<FinacExpenseCategorieFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacExpenseCategorieFormPage> {
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'parent_category_id': _parentCategoryIdController.text,
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacExpenseCategorieRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacExpenseCategorieRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacExpenseCategorieListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               const SizedBox(height: 16),
               TextFormField(controller: _parentCategoryIdController, decoration: const InputDecoration(labelText: 'ParentCategoryId')),
               const SizedBox(height: 16),
