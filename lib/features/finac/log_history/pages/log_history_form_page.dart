...
class FinacLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const FinacLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<FinacLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<FinacLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(finacLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(finacLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(finacLogHistoryListProvider);
