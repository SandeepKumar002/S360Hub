...
class FinacLogHistoryDashboardPage extends ConsumerWidget {
  const FinacLogHistoryDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(finacLogHistoryListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('LogHitorys (Finac)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/finac/log-history/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<FinacLogHistory>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: FinacLogHistoryDataSource(list, context),
...
class FinacLogHistoryDataSource extends DataGridSource {
  FinacLogHistoryDataSource(List<FinacLogHistory> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<FinacLogHistory>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as FinacLogHistory;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/finac/log-history/${item.id}'),
        child: Container(
