...
class ShieldLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const ShieldLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<ShieldLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<ShieldLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(shieldLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(shieldLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(shieldLogHistoryListProvider);
