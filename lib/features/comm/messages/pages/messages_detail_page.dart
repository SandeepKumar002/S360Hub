...
class CommMessageDetailPage extends ConsumerWidget {
  final String id;
  const CommMessageDetailPage({super.key, required this.id});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final itemAsync = ref.watch(commMessageDetailProvider(id));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => context.push('/comm/messages/$id/edit'),
          ),
        ],
      ),
      body: AsyncValueWidget<CommMessage>(
        value: itemAsync,
        data: (item) {
          return ListView(
            ...
            children: [
              ...
              ListTile(title: Text('SenderId'), subtitle: Text(item.senderId.toString())),
              ListTile(title: Text('Body'), subtitle: Text(item.body.toString())),
