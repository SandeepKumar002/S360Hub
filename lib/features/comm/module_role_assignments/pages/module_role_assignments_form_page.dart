...
class CommModuleRoleAssignmentFormPage extends ConsumerStatefulWidget {
  final String? id;
  const CommModuleRoleAssignmentFormPage({super.key, this.id});

  @override
  ConsumerState<CommModuleRoleAssignmentFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<CommModuleRoleAssignmentFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(commModuleRoleAssignmentRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(commModuleRoleAssignmentRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(commModuleRoleAssignmentListProvider);
