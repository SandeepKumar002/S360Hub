...
class TrainingLogHistoryDashboardPage extends ConsumerWidget {
  const TrainingLogHistoryDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(trainingLogHistoryListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('LogHitorys (Training)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/training/log-history/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<TrainingLogHistory>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: TrainingLogHistoryDataSource(list, context),
...
class TrainingLogHistoryDataSource extends DataGridSource {
  TrainingLogHistoryDataSource(List<TrainingLogHistory> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<TrainingLogHistory>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as TrainingLogHistory;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/training/log-history/${item.id}'),
        child: Container(
