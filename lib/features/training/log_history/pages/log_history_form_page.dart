...
class TrainingLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TrainingLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<TrainingLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TrainingLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(trainingLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(trainingLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(trainingLogHistoryListProvider);
