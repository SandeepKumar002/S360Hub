...
class TrainingAssessmentFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TrainingAssessmentFormPage({super.key, this.id});

  @override
  ConsumerState<TrainingAssessmentFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TrainingAssessmentFormPage> {
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'passing_score': double.tryParse(_passingScoreController.text),
    ...
    try {
       if (widget.id != null) {
         await ref.read(trainingAssessmentRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(trainingAssessmentRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(trainingAssessmentListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               const SizedBox(height: 16),
               TextFormField(controller: _passingScoreController, decoration: const InputDecoration(labelText: 'PassingScore')),
               const SizedBox(height: 16),
