...
class TrainingModuleRoleAssignmentFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TrainingModuleRoleAssignmentFormPage({super.key, this.id});

  @override
  ConsumerState<TrainingModuleRoleAssignmentFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TrainingModuleRoleAssignmentFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(trainingModuleRoleAssignmentRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(trainingModuleRoleAssignmentRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(trainingModuleRoleAssignmentListProvider);
