...
class TrainingFileDashboardPage extends ConsumerWidget {
  const TrainingFileDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(trainingFileListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Files (Training)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/training/files/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<TrainingFile>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: TrainingFileDataSource(list, context),
...
class TrainingFileDataSource extends DataGridSource {
  TrainingFileDataSource(List<TrainingFile> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<TrainingFile>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as TrainingFile;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/training/files/${item.id}'),
        child: Container(
