...
class TrainingModuleTeamMemberFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TrainingModuleTeamMemberFormPage({super.key, this.id});

  @override
  ConsumerState<TrainingModuleTeamMemberFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TrainingModuleTeamMemberFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(trainingModuleTeamMemberRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(trainingModuleTeamMemberRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(trainingModuleTeamMemberListProvider);
