...
class TrainingCourseFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TrainingCourseFormPage({super.key, this.id});

  @override
  ConsumerState<TrainingCourseFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TrainingCourseFormPage> {
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'duration_hours': double.tryParse(_durationHoursController.text),
    ...
    try {
       if (widget.id != null) {
         await ref.read(trainingCourseRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(trainingCourseRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(trainingCourseListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               const SizedBox(height: 16),
               TextFormField(controller: _durationHoursController, decoration: const InputDecoration(labelText: 'DurationHours')),
               const SizedBox(height: 16),
               TextFormField(controller: _levelController, decoration: const InputDecoration(labelText: 'Level')),
               const SizedBox(height: 16),
