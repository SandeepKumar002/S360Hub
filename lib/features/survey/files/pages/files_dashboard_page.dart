...
class SurveyFileDashboardPage extends ConsumerWidget {
  const SurveyFileDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(surveyFileListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Files (Survey)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/survey/files/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<SurveyFile>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: SurveyFileDataSource(list, context),
...
class SurveyFileDataSource extends DataGridSource {
  SurveyFileDataSource(List<SurveyFile> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<SurveyFile>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as SurveyFile;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/survey/files/${item.id}'),
        child: Container(
