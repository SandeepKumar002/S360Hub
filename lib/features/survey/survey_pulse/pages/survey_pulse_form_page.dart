...
class SurveySurveyPulseFormPage extends ConsumerStatefulWidget {
  final String? id;
  const SurveySurveyPulseFormPage({super.key, this.id});

  @override
  ConsumerState<SurveySurveyPulseFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<SurveySurveyPulseFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _isLoading = false;
  
  // Controllers
  final _frequencyController = TextEditingController();
  final _nextRunController = TextEditingController();
  final _targetGroupsController = TextEditingController();
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'next_run': _nextRunController.text,
      'target_groups': _targetGroupsController.text,
    ...
    try {
       if (widget.id != null) {
         await ref.read(surveySurveyPulseRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(surveySurveyPulseRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(surveySurveyPulseListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               TextFormField(controller: _nextRunController, decoration: const InputDecoration(labelText: 'NextRun')),
               const SizedBox(height: 16),
               TextFormField(controller: _targetGroupsController, decoration: const InputDecoration(labelText: 'TargetGroups')),
               const SizedBox(height: 16),
