...
class SurveyLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const SurveyLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<SurveyLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<SurveyLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(surveyLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(surveyLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(surveyLogHistoryListProvider);
