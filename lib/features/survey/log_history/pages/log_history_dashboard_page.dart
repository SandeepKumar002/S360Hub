...
class SurveyLogHistoryDashboardPage extends ConsumerWidget {
  const SurveyLogHistoryDashboardPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final listAsync = ref.watch(surveyLogHistoryListProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('LogHitorys (Survey)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () => context.push('/survey/log-history/new'),
          ),
        ],
      ),
      body: AsyncValueWidget<List<SurveyLogHistory>>(
        value: listAsync,
        data: (list) {
          return SfDataGrid(
            source: SurveyLogHistoryDataSource(list, context),
...
class SurveyLogHistoryDataSource extends DataGridSource {
  SurveyLogHistoryDataSource(List<SurveyLogHistory> data, this.context) {
    _dataGridRows = data.map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<SurveyLogHistory>(columnName: 'obj', value: e),
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    final item = row.getCells().first.value as SurveyLogHistory;
    return DataGridRowAdapter(cells: row.getCells().skip(1).map<Widget>((e) {
      return GestureDetector(
        onTap: () => context.push('/survey/log-history/${item.id}'),
        child: Container(
