...
class TaxforgeFileFormPage extends ConsumerStatefulWidget {
  final String? id;
  const TaxforgeFileFormPage({super.key, this.id});

  @override
  ConsumerState<TaxforgeFileFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<TaxforgeFileFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(taxforgeFileRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(taxforgeFileRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(taxforgeFileListProvider);
