...
class KnowledgeDocAccessControlFormPage extends ConsumerStatefulWidget {
  final String? id;
  const KnowledgeDocAccessControlFormPage({super.key, this.id});

  @override
  ConsumerState<KnowledgeDocAccessControlFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<KnowledgeDocAccessControlFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(knowledgeDocAccessControlRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(knowledgeDocAccessControlRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(knowledgeDocAccessControlListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               const SizedBox(height: 16),
               TextFormField(controller: _permissionController, decoration: const InputDecoration(labelText: 'Permission')),
               const SizedBox(height: 16),
