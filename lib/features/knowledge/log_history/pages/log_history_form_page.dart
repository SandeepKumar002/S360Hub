...
class KnowledgeLogHistoryFormPage extends ConsumerStatefulWidget {
  final String? id;
  const KnowledgeLogHistoryFormPage({super.key, this.id});

  @override
  ConsumerState<KnowledgeLogHistoryFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<KnowledgeLogHistoryFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(knowledgeLogHistoryRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(knowledgeLogHistoryRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(knowledgeLogHistoryListProvider);
