...
class KnowledgeDocTagFormPage extends ConsumerStatefulWidget {
  final String? id;
  const KnowledgeDocTagFormPage({super.key, this.id});

  @override
  ConsumerState<KnowledgeDocTagFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<KnowledgeDocTagFormPage> {
  ...
  Future<void> _save() async {
    ...
    try {
       if (widget.id != null) {
         await ref.read(knowledgeDocTagRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(knowledgeDocTagRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(knowledgeDocTagListProvider);
