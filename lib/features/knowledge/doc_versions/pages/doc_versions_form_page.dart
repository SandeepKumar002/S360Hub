...
class KnowledgeDocVersionFormPage extends ConsumerStatefulWidget {
  final String? id;
  const KnowledgeDocVersionFormPage({super.key, this.id});

  @override
  ConsumerState<KnowledgeDocVersionFormPage> createState() => _FormState();
}

class _FormState extends ConsumerState<KnowledgeDocVersionFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _isLoading = false;
  
  // Controllers
  final _docIdController = TextEditingController();
  final _versionNoController = TextEditingController();
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'version_no': int.tryParse(_versionNoController.text),
    ...
    try {
       if (widget.id != null) {
         await ref.read(knowledgeDocVersionRepositoryProvider).update(widget.id!, data);
         if (mounted) FeedbackService.showSuccess(context, 'Updated successfully');
       } else {
         await ref.read(knowledgeDocVersionRepositoryProvider).create(data);
         if (mounted) FeedbackService.showSuccess(context, 'Created successfully');
       }
       ref.invalidate(knowledgeDocVersionListProvider);
        ...
        child: Form(
          ...
          child: Column(
             children: [
               ...
               TextFormField(controller: _versionNoController, decoration: const InputDecoration(labelText: 'VersionNo')),
               const SizedBox(height: 16),
